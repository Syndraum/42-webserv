...
/*   Updated: 2021/06/30 18:08:20 by mchardin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	BuilderCore::skip_whitespaces()
{
	while(_line[_idx])
	{
...
void	BuilderCore::skip_comments()
{
	while(_line[_idx] == '#')
	{
		_idx += _line.find('\n') + 1;
...
void	BuilderCore::parse_server_port(Server *server)
{
	int	port;
	_idx += 7;
	skip_whitespaces();
...
void	BuilderCore::parse_server_name(Server *server)
{
	_idx += 12;
	skip_whitespaces();
...
void	BuilderCore::parse_server_root(Server *server)
{
	_idx += 5;
	skip_whitespaces();
	size_t	len = _line.find_first_of(";} \n\r\t\v\f", _idx) - _idx;
...
void	BuilderCore::parse_server_path_error_page(Server *server)
{
	_idx += 16;
	skip_whitespaces();
	size_t	len = _line.find_first_of(";} \n\r\t\v\f", _idx) - _idx;
...
void	BuilderCore::parse_server_auto_index(Server *server)
{
	_idx += 10;
	skip_whitespaces();
...
void	BuilderCore::parse_server_client_max_body_size(Server *server)
{
	_idx += 21;
	skip_whitespaces();
	int	client_max_body_size = stoi_skip_number();
...
BuilderCore::BuilderCore(std::istream &fd, Core *core) : _idx(0), _core(core)
{
...
int		BuilderCore::stoi_skip_number()
{
	int		ret;
	int		i = 0;
...
Core	*BuilderCore::get_builded_core() const
{
	return (_core);
}

void	BuilderCore::print_debug() const
{
