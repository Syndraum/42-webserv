...
/*   Updated: 2021/06/25 18:03:12 by mchardin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	CoreBuilder::parse_server_autoindex(Server *server)
{
	_idx += 10;
	skip_whitespaces();
...
void	CoreBuilder::parse_server_body_size(Server *server)
{
	int	body_size;
	_idx += 21;
	skip_whitespaces();
	try { body_size = std::stoi(&_line[_idx], &_idx); }
	catch (std::exception &e) { throw (std::exception); }
	server->set_body_size(body_size);
	skip_whitespaces();
	if (_line[_idx] != ';' && _line[_idx] != '}')
		throw (std::exception);
	if(_line[_idx] == ';')
...
void	CoreBuilder::parse_server()
{
	...
	while (_line[_idx] != '}')
	{
		...
		// 	parse_server_string(server, 6, server->set_index());
			...
			parse_server_autoindex(server);
		else if (!_line.compare(_idx, 21, "client_max_body_size "))
			parse_server_body_size(server);
